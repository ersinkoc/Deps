/**
 * Unused plugin - Detect unused dependencies
 *
...
import { glob, ignore } from '../../utils/glob.js';
...
/**
 * Unused dependency detection plugin
 */
export const unusedPlugin: AnalyzerPlugin<AnalyzerContext> = {
  name: 'unused',
  version: '1.0.0',

  install(kernel) {
    kernel.on('analyze', async (context) => {
      kernel.reportProgress('unused', 0, 'Scanning for unused dependencies');
      ...
      if (allDeps.size === 0) {
        kernel.reportProgress('unused', 100, 'No dependencies to check');
        (kernel as any).setResult('unused', []);
        return;
      }
      ...
      // Store result
      (kernel as any).setResult('unused', unused);

      // Report findings
      for (const dep of unused) {
        kernel.reportFinding('unused', 'warning', `Unused dependency: ${dep}`, dep);
      }

      kernel.reportProgress('unused', 100, `Found ${unused.length} unused dependencies`);
    });
  }
};
